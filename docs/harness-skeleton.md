...
A should-not-fire case is graded only on one skill staying quiet, so truncation can hide nothing unless that skill fires after another one. [calibration.md](calibration.md) measured that it never did, across 65 negative and watch-list runs, and [#17](https://github.com/MalcolmMcNeely/skills-marketplace/issues/17) switched the rule on for that half. That is 50 runs of a 125-run pass, at about 9.5 seconds each instead of about 40. A killed run still emits no `result` line and cannot report its own cost, so [#16](https://github.com/MalcolmMcNeely/skills-marketplace/issues/16) charges it a fixed estimate, and every report that quotes the figure says it is one.
...
- **Whether the `FirstDecision` stop rule is safe for negative cases.** A negative case is graded only on one skill staying quiet, so truncation is harmless there and the saving is real. It was not run that way. **Settled since**: the calibration pass measured that `csharp-new-class` never fired after another skill across 65 negative and watch-list runs, and [#17](https://github.com/MalcolmMcNeely/skills-marketplace/issues/17) switched the rule on for should-not-fire cases on 9 September 2026. See [calibration.md](calibration.md).
