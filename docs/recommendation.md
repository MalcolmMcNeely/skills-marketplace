...
| 3. Firing accuracy | Our harness, modelled on `run_eval.py` | 125 planned runs per engine, 133 measured at `$30.01` | PRs touching a description | Undecided, see below |
...
| 4. Contract assertions | Same harness, invoking the skill **by name** | 5 runs per case, at a measured `$0.244` a run | PRs touching a body | Undecided, see below |
...
Layers 1 and 2 catch most real breakage for nothing. They are built, and `.github/workflows/free-gate.yml` runs them on every push to `main` and every pull request.
...
**Set the should-fire threshold from measured behaviour, not from an expectation of 100%.** A well-specified, unambiguous cross-stack request loaded the right skills in only 10 runs of 15, and no description wording tested removed the misses. A failing run fired nothing rather than the wrong thing. A gate demanding a perfect trigger rate will fail on healthy skills. Calibrate against a baseline measured on the frozen good fixture, then set the bar as the highest value a good skill clears 95% of the time. Recompute it on any model, CLI or catalogue change. [Scoring](scoring.md) carries the formula and [calibration.md](calibration.md) carries the pass that ran it: 60 of 60 on 8 September 2026, so the gate at 60 runs is 53. [breakage.md](breakage.md) replicated that 60 of 60 on 9 September 2026, so the gate rests on 120 runs, 60 in each pass. Note what that pass also showed. A perfect score must not set a perfect gate, so the bar comes from the interval's lower bound and not from the score itself.
...
Budget per full pass, at twelve engines: the pass in [calibration.md](calibration.md) ran [#10](https://github.com/MalcolmMcNeely/skills-marketplace/issues/10)'s 125-run suite end to end. Eight void runs took it to 133 journalled runs, for **$30.01** in 02:08:14. A twelve-engine nightly is nearer **$360** than $43. The earlier $0.043 was measured on read-only probes with writes disallowed, and does not apply to a run that creates files. [Harness skeleton](harness-skeleton.md) carries the per-run measurement.

**Every dollar figure above is notional.** It is what the run would cost on the API. Every measurement on this map was made on a laptop against a Claude Code subscription, where no cash moves and the run draws on usage limits instead. The `result` line reports the same number either way, so nothing in the harness can tell the two apart. Treat these figures as a size comparison between run shapes, not as a bill. [#7](https://github.com/MalcolmMcNeely/skills-marketplace/issues/7) ruled the API key and the CI that would have needed it out of scope; [running the paying layers](running-the-paid-layers.md) carries the reasoning and the constraint that replaces money, which is time.

**Where layer 3 runs is not settled.** At a measured $30.01 and 128 minutes a pass, blocking every description pull request needs a decision it has not had. The pass mark is no longer the objection: [calibration.md](calibration.md) replaced the borrowed 0.67 with a measured gate of 53 of 60. [#11](https://github.com/MalcolmMcNeely/skills-marketplace/issues/11) ruled that policy out of scope until there is a real catalogue to act on. Until then the paying layers run on demand only, on a developer's machine, with `--max-budget-usd 0.60` per run and a suite ceiling of $50 as runaway guards.
