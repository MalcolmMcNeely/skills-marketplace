...
public sealed record BreakageArm
{
    ...
    /// <summary>Laid over fixtures/good for layer 4. Null runs the unbroken plugin.</summary>
...
/// <summary>Runs one arm: the firing half against an overlaid catalogue, then the contract half.</summary>
public sealed class BreakagePass(HarnessPaths paths, SuiteFile suite)
{
        ...
        Action<string>? log = null, CancellationToken ct = default)
    {
        ...
        if (arm.RunFiring)
        {
            ...
            var pass = new CalibrationPass(paths, suite, new FiringRunner(paths, catalogue), arm.FiringShape);
    ...
    {
        var started = DateTimeOffset.UtcNow;
        var plugin = builder.Build(paths.GoodPlugin, Overlay(arm.ContractOverlay));
        var runner = new ContractRunner(paths);

        foreach (var c in suite.Contract)
        {
            ...
            {
                var outcome = await runner.RunAsync(suite.SkillUnderTest, c.Task, plugin, token);
                var score = Scoring.ScoreContract(outcome, suite.SkillUnderTest, assertions);
    ...
    private string? Overlay(string? name) => name is null ? null : paths.BreakOverlay(name);
}
